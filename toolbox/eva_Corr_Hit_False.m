function [cor,hit,false] = eva_Corr_Hit_False(true_sources,true_source_locs,reconstr_sources,nd,neighbors)% CORR_HIT_FASLSE computes some measures related to comparation between
% ture sources and sources reconstruted by one algorithm, including BMN and
% MxNE
% Input Parameters:
...
power = compute_power(reconstr_sources,1);
...
thresh1 = a(floor(size(estpower,2) *per)); % 10 per cent of <a>
thresh2 = max_power * 0.05;   % 0.05 for lcmv, 0.05 for sbl-bf, 0.05 for champ, 0.2 for bmn, 0.05 for mxne
    
thresh = max(thresh1,thresh2);
false_thresh = thresh;
hit_thresh = thresh;

NB = 30; % 30 closest pointers
...
max_false = size(reconstr_sources,1)/ nd * per - sum(hit2);
