...
function [gamma,x,w,c,l,sigu]=awsm_champ_noiseup(y,f,nem,nd,vcs,plot_on,coup,ncf)
...
eps1=1e-8;
[nk nvd]=size(f);
nv=nvd/nd; % # of sources
nt=size(y,2); % # of sample pointers
...
%    iem
    ...
    invc = inv(c); % makea row of code  above a comment to ensure the result be not complex
    
    like(iem) = -0.5 * (sum(log(max(d,eps1))) + nk * log(2 * pi))- 0.5 * sum(sum(invc.*cyy));  %4.77 4.18 y's distribution directly
...
%         igam(ff) = 1./vvec(ff);
%         
        igam = (ones(nvd,1)-sum(w.*f',2)).*vvec;
   ...
   if (abs(norm(vvec_old)-norm(vvec))<1e-10) || abs(like_old-like(iem))<1e-8
       break;
   end
    like_old = like(iem);
end
